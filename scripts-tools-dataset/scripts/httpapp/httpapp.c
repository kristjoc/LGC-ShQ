...
struct HTTP_PUT_HEADER parse_put_header(const char *header)
{
        ...
        if (pos)
                sscanf(pos, "%*s %s", fname);

        pos = strstr(header, "Content-length:");
        if (pos)
                sscanf(pos, "%*s %ld", &put.content_length);

        pos = strstr(header, "User-Agent:");
        if (pos)
                sscanf(pos, "%*s %ld", &put.thread_id);

        snprintf(put.filename, sizeof(put.filename), "%s", fname);
        int len = strlen(put.filename);
        snprintf(put.filename + len, sizeof(put.filename) - len, "%ld", put.thread_id);
