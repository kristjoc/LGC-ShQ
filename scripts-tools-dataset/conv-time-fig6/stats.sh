...
extract_conv_time_over_convergence() {
    filename=("cwnd_${MODE}_hull_" "cwnd_${MODE}_shq_")
    for i in ${!chosen_rate[@]}; do
	rate=${chosen_rate[$i]}
        for f in $flows_no; do
            # echo -n $f >> qdelay_over_nflows.dat
            for j in ${!filename[@]}; do
                myfilenames=`ls | grep ${filename[$j]}${rate}_flows_${f}_run`
                for eachfile in $myfilenames; do
                    if [[ "$eachfile" == *"link"* ]]; then
                        # fair share cwnd value
                    elif [[ "$eachfile" == *"zelda"* ]]; then
                    elif [[ "$eachfile" == *"hylia"* ]]; then
                    elif [[ "$eachfile" == *"ganon"* ]]; then
                    elif [[ "$eachfile" == *"epona"* ]]; then
