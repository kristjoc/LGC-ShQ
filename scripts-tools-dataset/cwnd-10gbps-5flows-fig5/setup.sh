...
old_setup_aqm() {
    CMD="sudo tc qdisc add dev 10Gf root handle 2: netem delay 1ms;
         ...
         sudo tc class add dev 10Gf parent 3: classid 10 htb rate 10gbit ceil \
10gbit burst 1514b cburst 1514b"

    # CMD="sudo tc qdisc add dev 10Gf root handle 1: netem delay 5ms"
    ssh midna $CMD
#     CMD_HULL="sudo tc qdisc add dev 10Gf parent 1:1 handle 10: hull limit 1514000b \
# drate ${RATE}mbit markth 30kb"
#     CMD_SHQ="sudo tc qdisc add dev 10Gf parent 1:1 handle 10: shq limit 1000 \
# interval 1ms maxp 0.75 alpha 0.95 bandwidth ${RATE}mbit ecn"

    CMD_HULL="sudo tc qdisc add dev 10Gf parent 3:10 handle 15: hull limit 1514000b \
drate ${RATE}mbit markth 150kb"
...
setup_aqm() {
    ...
    CMD_HULL="sudo tc qdisc add dev 10Gf parent 1:1 handle 10: hull limit \
1500000b drate ${RATE}mbit markth 175kb"
    CMD_SHQ="sudo tc qdisc add dev 10Gf parent 1:1 handle 10: shq limit 1000 \
interval 1ms maxp 0.8 alpha 0.95 bandwidth ${RATE}mbit ecn"
